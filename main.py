from fastapi import FastAPI, File, UploadFile, HTTPException, Form, Header, Request, Body, Depends, BackgroundTasks, status
...
async def recognize_image(request: RecognitionRequest):
    ...
    try:
        ...
        try:
            ...
            objects = [label.description for label in sorted_labels]
        ...
        # Visual captioning with Gemini Pro Vision - Enhanced to identify specific products
        scene_description = ""
        specific_products = []
        try:
            # Configure the model
            ...
            # Generate detailed caption with product identification
            prompt = """
            Please analyze this image and provide:
            
            1. A detailed description of what you see in the image (one paragraph)
            2. Identify any specific products or models visible, being as precise as possible with brand names, model numbers, etc.
            
            For example, instead of just saying "a car", specify "a black 1968 Dodge Charger R/T" if that level of detail is visible.
            Instead of just "a phone", specify "iPhone 14 Pro in gold color" if identifiable.
            
            Format your response as:
            DESCRIPTION: [Your detailed description]
            PRODUCTS: [comma-separated list of specific products identified, if any]
            """
            
            response = model.generate_content([prompt, image_parts[0]])
            gemini_response = response.text.strip()
            
            # Process Gemini response to extract description and products
            if "DESCRIPTION:" in gemini_response:
                parts = gemini_response.split("PRODUCTS:")
                if len(parts) >= 2:
                    description_part = parts[0].replace("DESCRIPTION:", "").strip()
                    products_part = parts[1].strip()
                    
                    scene_description = description_part
                    
                    # Extract specific products from the products section
                    if products_part and products_part.lower() != "none" and products_part.lower() != "n/a":
                        product_list = [p.strip() for p in products_part.split(",")]
                        specific_products = [p for p in product_list if p and len(p) > 3]
                else:
                    scene_description = parts[0].replace("DESCRIPTION:", "").strip()
            else:
                scene_description = gemini_response
            
            # If no products section was found, try to extract products from the description
            if not specific_products and scene_description:
                # Look for specific patterns in the description
                # Year + Make + Model pattern for vehicles
                year_make_model_matches = re.findall(r'(19|20)\d{2}\s+[A-Z][a-zA-Z]+\s+[A-Z][a-zA-Z]+', scene_description)
                if year_make_model_matches:
                    specific_products.extend(year_make_model_matches)
                
                # Brand + Model pattern
                brand_model_matches = re.findall(r'\b([A-Z][a-zA-Z]+\s+[A-Z][a-zA-Z]+(\s+[A-Z][a-zA-Z]+)?)\b', scene_description)
                if brand_model_matches:
                    for match in brand_model_matches:
                        product = match[0]
                        if product not in specific_products and not any(product in p for p in specific_products):
                            specific_products.append(product)
                
                # Model with numbers
                model_number_matches = re.findall(r'\b([A-Za-z]+\s+\d+(\s+[A-Za-z]+)?)\b|\b([A-Z]+\d{3,4})\b', scene_description)
                if model_number_matches:
                    for match in model_number_matches:
                        if match[0] and match[0] not in specific_products:
                            specific_products.append(match[0])
                        elif match[2] and match[2] not in specific_products:
                            specific_products.append(match[2])
                            
            # Add specific products to the beginning of objects list if they're not already there
            for product in specific_products:
                if product not in objects:
...
async def search_products(request: ProductRequest):
    ...
    try:
        ...
        # Extract specific details from description if available
        specific_product_details = []
        if hasattr(request, 'description') and request.description:
            description = request.description
            
            # Look for specific patterns in the description
            # Year + Make + Model pattern for vehicles (e.g., "1968 Dodge Charger")
            year_make_model_matches = re.findall(r'(19|20)\d{2}\s+[A-Z][a-zA-Z]+\s+[A-Z][a-zA-Z]+', description)
            if year_make_model_matches:
                specific_product_details.extend(year_make_model_matches)
            
            # Brand + Model pattern (e.g., "Sony PlayStation 5")
            brand_model_matches = re.findall(r'\b([A-Z][a-zA-Z]+\s+[A-Z][a-zA-Z]+(\s+[A-Z][a-zA-Z]+)?)\b', description)
            if brand_model_matches:
                specific_product_details.extend([match[0] for match in brand_model_matches])
            
            # Model with numbers (e.g., "iPhone 14 Pro" or "GTX 3080")
            model_number_matches = re.findall(r'\b([A-Za-z]+\s+\d+(\s+[A-Za-z]+)?)\b|\b([A-Z]+\d{3,4})\b', description)
            if model_number_matches:
                for match in model_number_matches:
                    if match[0]:
                        specific_product_details.append(match[0])
                    elif match[2]:
                        specific_product_details.append(match[2])
            
            # Check if the search term is already a specific product detail
            is_specific_product = any(detail.lower() in search_term.lower() or search_term.lower() in detail.lower() for detail in specific_product_details)
            
            # If the search term is a generic term but we found specific products, prioritize those
            if not is_specific_product and specific_product_details and len(search_term.split()) < 3:
                # Check if the search term is a general category like "car", "phone", etc.
                general_categories = ["car", "phone", "camera", "laptop", "watch", "tv", "television", "computer"]
                if any(category.lower() in search_term.lower() for category in general_categories):
                    # Use the most specific product detail as the primary search term
                    enhanced_term = specific_product_details[0]
                    print(f"Using specific product '{enhanced_term}' instead of general term '{search_term}'")
                    search_term = enhanced_term
                else:
                    # Add the search term to the specific product for context
                    enhanced_term = f"{search_term} {specific_product_details[0]}"
                    ...
                    if vision_client and google_cloud_initialized:
                        ...
                        if hasattr(web_detection, 'web_detection'):
                            ...
                            if product_terms:
                                ...
                                if len(search_term.split()) < 3:  # Only enhance short search terms
                                    enhanced_term = f"{search_term} {product_terms[0]}"
                                    print(f"Enhanced search term with web entity: {enhanced_term}")
                                    search_term = enhanced_term
        ...
        # Check if it's a specific product (e.g., "1968 Dodge Charger", "iPhone 14 Pro")
        has_specific_model = bool(re.search(r'(19|20)\d{2}\s+[A-Z][a-z]+\s+[A-Z][a-z]+|[A-Z][a-z]+\s+\d+(\s+[A-Z][a-z]+)?', search_term))
        
        if has_specific_model:
            # For specific products, create more precise search links
            ...
            # Add specialized links for certain types of products
            if "car" in search_term.lower() or re.search(r'(19|20)\d{2}', search_term):
                products.extend([
                    {
                        "name": f"{product_name} - Carvana",
                        "url": f"https://www.carvana.com/cars/search?query={product_name.replace(' ', '%20')}",
                        "price": "Search",
                        "store": "Carvana",
                        "score": 0.8
                    },
                    {
                        "name": f"{product_name} - AutoTrader",
                        "url": f"https://www.autotrader.com/cars-for-sale/all-cars?searchText={product_name.replace(' ', '%20')}",
                        "price": "Search",
                        "store": "AutoTrader",
                        "score": 0.75
                    }
                ])
            elif "phone" in search_term.lower() or "iphone" in search_term.lower():
                products.extend([
                    {
                        "name": f"{product_name} - Best Buy",
                    ...
                    {
                        "name": f"{product_name} - Swappa",
                        "url": f"https://swappa.com/search?q={product_name.replace(' ', '+')}",
                        "price": "Search",
                        "store": "Swappa",
                        "score": 0.75
                    }
...
class SerpSearchRequest(BaseModel):
    query: str
    search_type: Optional[str] = "web"  # Can be "web", "images", or "videos"

class SerpSearchResponse(BaseModel):
    success: bool
    organic_results: Optional[List[Dict[str, Any]]] = []
    images_results: Optional[List[Dict[str, Any]]] = []
    videos_results: Optional[List[Dict[str, Any]]] = []
    short_videos: Optional[List[Dict[str, Any]]] = []
    found_in_videos: Optional[List[Dict[str, Any]]] = []
    related_questions: Optional[List[Dict[str, Any]]] = []
    suggested_searches: Optional[List[Dict[str, Any]]] = []
    shopping_results: Optional[List[Dict[str, Any]]] = []
    related_searches: Optional[List[Dict[str, Any]]] = []
    error: Optional[str] = None

class GoogleLensRequest(BaseModel):
    url: str
...
class GoogleLensResponse(BaseModel):
    ...
    exact_matches_page_token: Optional[str] = None
    visual_matches: List[Dict[str, Any]] = []
    visual_matches_page_token: Optional[str] = None
    products: List[Dict[str, Any]] = []
    products_page_token: Optional[str] = None
    
class GoogleLensExactRequest(BaseModel):
    page_token: str
    image_url: Optional[str] = None
    
class GoogleLensExactResponse(BaseModel):
    exact_matches: List[Dict[str, Any]] = []
    page_token: Optional[str] = None

class GoogleLensVisualRequest(BaseModel):
    page_token: str
    image_url: Optional[str] = None
    
class GoogleLensVisualResponse(BaseModel):
    visual_matches: List[Dict[str, Any]] = []
    page_token: Optional[str] = None

class GoogleLensProductsRequest(BaseModel):
    page_token: str
    image_url: Optional[str] = None
    
class GoogleLensProductsResponse(BaseModel):
    products: List[Dict[str, Any]] = []
    page_token: Optional[str] = None
...
def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> User:
    ...
    try:
        ...
        # You can fetch more user details from Supabase if needed
...
async def root():
    ...
    """
    return {"message": "Welcome to LensLynx API!", "status": "operational"}
...
async def recognize_image(
    ...
    try:
        ...
        # Download the image or get it from URL
        if request.image_url.startswith(('http://', 'https://')):
            image = vision_types.Image()
            image.source.image_uri = request.image_url
        else:
            # Base64 or local file handling would go here
            raise HTTPException(status_code=400, detail="Only URL-based images are supported")
        
        # Label detection
        if request.detect_objects:
            label_response = vision_client.label_detection(image=image)
            response.labels = [label.description for label in label_response.label_annotations]
        ...
        if request.analyze_text:
            ...
            if text_response.text_annotations:
                response.text = text_response.text_annotations[0].description
        
        # Image properties
        if request.generate_description:
            # This is a simplified approach - in production you might want 
            # to use a more sophisticated method to generate descriptions
            if response.labels:
                response.description = f"This image contains: {', '.join(response.labels[:5])}"
        
        # Save the result to Supabase
        supabase.table("recognition_results").insert({
...
@app.post("/google-lens", response_model=GoogleLensResponse)
async def search_google_lens(
    request: GoogleLensRequest
):
    """
    ...
    try:
        url = request.url
        print(f"Searching Google Lens for image: {url}")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if e.response.status_code == 429:
                    raise HTTPException(
                        status_code=429, 
                        detail="Rate limit exceeded for Google Lens API. Please try again later."
                    )
                else:
                    raise e
        
        print(f"Google Lens search successful. Response keys: {data.keys()}")
        
        # Extract results
        result = GoogleLensResponse(success=True)
        
        # Extract exact matches
        if "visual_matches" in data:
            # In SerpAPI, exact matches are part of visual_matches
            exact_matches = []
            visual_matches = []
            
            for match in data["visual_matches"]:
                if match.get("source") == "Exact match":
        ...
        if "serpapi_pagination" in data:
            if "next" in data["serpapi_pagination"]:
                # Extract page tokens from the URL
                next_url = data["serpapi_pagination"]["next"]
                query_params = {k: v for k, v in [p.split('=') for p in next_url.split('?')[1].split('&')]}
                
                page_token = query_params.get("page", None)
                if page_token:
                    if exact_matches:
                        result.exact_matches_page_token = page_token
                    if visual_matches:
                        result.visual_matches_page_token = page_token
        
        # Add tokens from the response data directly if available
        if "products_page_token" in data:
            result.products_page_token = data["products_page_token"]
        
        if "exact_matches_page_token" in data:
            result.exact_matches_page_token = data["exact_matches_page_token"]
        
        if "visual_matches_page_token" in data:
            result.visual_matches_page_token = data["visual_matches_page_token"]
            
        # Extract product results
        if "shopping_results" in data:
            result.products = data["shopping_results"]
        
        # Add all results for general search
        if "visual_matches" in data:
            result.search_results = data["visual_matches"]
        ...
        print(f"Error in search_google_lens: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error searching Google Lens: {str(e)}")

@app.post("/google-lens-exact", response_model=GoogleLensExactResponse)
async def search_google_lens_exact(
    request: GoogleLensExactRequest
):
    """
    Get exact matches from Google Lens using SerpAPI with pagination
    """
    try:
        print(f"Searching Google Lens exact matches with page token: {request.page_token}")
        ...
        # When using page_token, we should NOT include the url parameter
        # as per SERP API documentation
        ...
        print(f"Google Lens exact match search successful. Response keys: {data.keys()}")
        
        # Extract results
        result = GoogleLensExactResponse()
        
        # Extract exact matches - can be either in 'exact_matches' directly or inside 'visual_matches'
        if "exact_matches" in data:
            # Use exact_matches directly from the response if available
            result.exact_matches = data["exact_matches"]
        elif "visual_matches" in data:
            # Otherwise, filter from visual_matches
            exact_matches = [
                match for match in data["visual_matches"]
                if match.get("source") == "Exact match"
            ]
            result.exact_matches = exact_matches
        ...
        if "exact_matches_page_token" in data:
            result.page_token = data["exact_matches_page_token"]
        ...
        print(f"Error in search_google_lens_exact: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error searching Google Lens exact matches: {str(e)}")

@app.post("/google-lens-visual", response_model=GoogleLensVisualResponse)
async def search_google_lens_visual(
    request: GoogleLensVisualRequest
):
    """
    Get visual matches from Google Lens using SerpAPI with pagination
    """
    try:
        print(f"Searching Google Lens visual matches with page token: {request.page_token}")
        ...
        print(f"Google Lens visual match search successful. Response keys: {data.keys()}")
        
        # Extract results
        result = GoogleLensVisualResponse()
        
        # Extract visual matches - the API can return them in different ways
        visual_matches = []
        
        # Try to get from different possible response formats
        if "visual_matches" in data:
            # Check if we need to filter out exact matches
            if any(match.get("source") == "Exact match" for match in data["visual_matches"]):
        ...
        # Get next page token if available
        if "visual_matches_page_token" in data:
            result.page_token = data["visual_matches_page_token"]
        ...
        print(f"Error in search_google_lens_visual: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error searching Google Lens visual matches: {str(e)}")

@app.post("/google-lens-products", response_model=GoogleLensProductsResponse)
async def search_google_lens_products(
    request: GoogleLensProductsRequest
):
    """
    Get product matches from Google Lens using SerpAPI with pagination
    """
    try:
        print(f"Searching Google Lens products with page token: {request.page_token}")
        ...
        print(f"Google Lens product search successful. Response keys: {data.keys()}")
        
        # Extract results
        result = GoogleLensProductsResponse()
        products = []
        
        # Check all possible places where products might be
        if "shopping_results" in data:
        ...
        # If no dedicated product results, extract from visual matches
        if not products and "visual_matches" in data:
            # Filter visual matches that look like products (have price or shop in source)
            product_matches = [
                match for match in data["visual_matches"]
                if match.get("price") or (match.get("source") and "shop" in match.get("source", "").lower())
            ]
            print(f"Found {len(product_matches)} product-like items in visual matches")
            products = product_matches
            
            # If still no products, just use visual matches that have thumbnails
            if not products:
                product_matches = [
                    match for match in data["visual_matches"]
                    if match.get("thumbnail")
                ]
                print(f"Using {len(product_matches)} visual matches as products")
                products = product_matches
        
        result.products = products
        print(f"Returning {len(products)} products in total")
        
        # Get next page token if available
        if "products_page_token" in data:
            result.page_token = data["products_page_token"]
        ...
        print(f"Error in search_google_lens_products: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error searching Google Lens products: {str(e)}")
...
async def search_serp(
    ...
    try:
        query = request.query
        print(f"Searching SerpAPI for query: {query}")
        
        # Build parameters for SerpAPI
        params = {
            "engine": "google",  # Default to standard web search
            "q": query,
            "api_key": SERP_API_KEY,
        }
        
        # Select the appropriate search engine based on search_type
        if request.search_type == "images":
            params["engine"] = "google_images"
        elif request.search_type == "videos":
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                response = await client.get(SERP_URL, params=params, timeout=30.0)
                response.raise_for_status()
                data = response.json()
            except httpx.HTTPStatusError as e:
                print(f"HTTP error in search_serp: {e.response.status_code} - {str(e)}")
                if e.response.status_code == 429:
                    raise HTTPException(
                        status_code=429, 
                        detail="Rate limit exceeded for SerpAPI. Please try again later."
                    )
                else:
                    raise HTTPException(
                        status_code=e.response.status_code,
                        detail=f"Error from SerpAPI: {str(e)}"
                    )
            except httpx.TimeoutException:
                print("Timeout error in search_serp")
                raise HTTPException(
                    status_code=504,
                    detail="Request to SerpAPI timed out. Please try again later."
                )
            except httpx.RequestError as e:
                print(f"Request error in search_serp: {str(e)}")
                raise HTTPException(
                    status_code=500,
                    detail=f"Error making request to SerpAPI: {str(e)}"
                )
        
        print(f"SerpAPI search successful. Response keys: {data.keys()}")
        
        # Extract results
        result = SerpSearchResponse(success=True)
        
        # Extract organic results for web search
        if "organic_results" in data:
            organic_results = data["organic_results"]
            for result_item in organic_results:
                if "rich_snippet" in result_item:
                    rich_snippet = result_item["rich_snippet"]
                    
                    # Fix top extensions if it exists
                    if "top" in rich_snippet and rich_snippet["top"] is not None:
                        if "extensions" in rich_snippet["top"] and not isinstance(rich_snippet["top"]["extensions"], list):
                            rich_snippet["top"]["extensions"] = [str(rich_snippet["top"]["extensions"])]
                        elif "extensions" not in rich_snippet["top"]:
                            rich_snippet["top"]["extensions"] = []
                            
                        # Ensure detected_extensions is a list
                        if "detected_extensions" in rich_snippet["top"] and not isinstance(rich_snippet["top"]["detected_extensions"], list):
                            detected_ext = rich_snippet["top"]["detected_extensions"]
                            if isinstance(detected_ext, dict):
                                # Convert dict to list of key-value pairs
                                rich_snippet["top"]["detected_extensions"] = [f"{k}: {v}" for k, v in detected_ext.items()]
                            else:
                                rich_snippet["top"]["detected_extensions"] = [str(detected_ext)]
                        elif "detected_extensions" not in rich_snippet["top"]:
                            rich_snippet["top"]["detected_extensions"] = []
                    
                    # Fix bottom extensions if it exists
                    if "bottom" in rich_snippet and rich_snippet["bottom"] is not None:
                        if "extensions" in rich_snippet["bottom"] and not isinstance(rich_snippet["bottom"]["extensions"], list):
                            rich_snippet["bottom"]["extensions"] = [str(rich_snippet["bottom"]["extensions"])]
                        elif "extensions" not in rich_snippet["bottom"]:
                            rich_snippet["bottom"]["extensions"] = []
                            
                        # Ensure detected_extensions is a list
                        if "detected_extensions" in rich_snippet["bottom"] and not isinstance(rich_snippet["bottom"]["detected_extensions"], list):
                            detected_ext = rich_snippet["bottom"]["detected_extensions"]
                            if isinstance(detected_ext, dict):
                                # Convert dict to list of key-value pairs
                                rich_snippet["bottom"]["detected_extensions"] = [f"{k}: {v}" for k, v in detected_ext.items()]
                            else:
                                rich_snippet["bottom"]["detected_extensions"] = [str(detected_ext)]
                        elif "detected_extensions" not in rich_snippet["bottom"]:
        ...
        if "images_results" in data:
            result.images_results = data["images_results"]
        
        # Extract video results (using the Google Videos API)
        if "video_results" in data:
            result.videos_results = data["video_results"]
        elif "videos_results" in data:
            result.videos_results = data["videos_results"]
        
        # Extract short videos if available
        if "short_videos" in data:
            result.short_videos = data["short_videos"]
            
        # Extract found in videos if available
        if "found_in_videos" in data:
            result.found_in_videos = data["found_in_videos"]
        ...
        if "suggested_searches" in data:
            result.suggested_searches = data["suggested_searches"]
            
        # Extract shopping results if available
        if "shopping_results" in data:
            result.shopping_results = data["shopping_results"]
            
        # Extract related searches
        if "related_searches" in data:
            result.related_searches = data["related_searches"]
        ...
        print(f"Error in search_serp: {str(e)}")
        traceback.print_exc()  # Print full traceback for debugging
        return SerpSearchResponse(
            success=False,
            error=f"Error searching SerpAPI: {str(e)}"
        )
